...
### 5. Release gating uses one authoritative workflow — Medium (external bootstrap pending)

- [ ] **Status: implementation complete; external protection bootstrap and empirical check proof remain fail-closed prerequisites.** `.github/workflows/verification.yml` remains the reusable authority. CI exposes `Required verification`; full-SHA-pinned actionlint, parsed structural/mutation tests, strict artifacts/installer checks, and the bounded Linux inventory remain enforced.
- Initial admission requires the remote tag, event SHA, and `HEAD` to equal the freshly fetched current `origin/main` tip. Later pre-draft/pre-publication rechecks require that unchanged commit to remain on protected main's first-parent history, so normal main advancement does not brick an admitted run. Reruns need read-only API proof of successful initial admission in an earlier attempt of the same run/SHA. Tagged metadata remains strict. Live audits use only `RELEASE_PROTECTION_AUDIT_TOKEN` and reject missing credentials or canonical main/tag/CODEOWNERS drift.
