...
python3 -c 'import json,sys; v=json.load(sys.stdin); assert v["type"] == "private_accepted" and v["protocol_version"] == 3 and len(v["request_id"]) == 32; assert v["operation_id"] == v["message_id"]; assert v["acceptance_acknowledged"] is True and v["duplicate_accepted"] is False and v["durable"] is False and v["read"] is False; assert v["body_bytes"] == int(sys.argv[1]) and "body" not in v; assert len(v["message_id"]) == 32' "${#PRIVATE}" \
