...
python3 -c 'import json,sys; v=json.load(sys.stdin); assert v["type"] == "private_accepted" and v["schema_version"] == 1; assert v["acceptance_acknowledged"] is True and v["durable"] is False and v["read"] is False; assert v["body_bytes"] == int(sys.argv[1]) and "body" not in v; assert len(v["message_id"]) == 32' "${#PRIVATE}" \
