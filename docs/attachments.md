...
Copy the `offer` value from the `share` or `listen` JSON output. Signed offers require one canonical lowercase operation-format ID shared by the EnvelopeV2 message ID and embedded offer ID, a canonical raw ticket whose provider and content hash/format are signed, a safe typed name/kind, the signed size and timestamp, and the expected topic. The full signed relationships are revalidated at wire, IPC, HTTP/SSE registration, saved download-token, and generated-event boundaries; malformed offers are rejected before accepted-traffic or replay/rate admission, subscriber fanout, download registration, network/export work, or pin/tag creation (they still pay the separate cheap pre-verification attempt budget). Persistent tags do not retain the signed envelope and therefore cannot revalidate its signature, topic, timestamp, ticket, or body. Tag parsing instead validates only the canonical provider, offer ID, direction/kind, and encoded name fields that the tag actually stores. Live wire and IPC/HTTP events require a nonzero signed timestamp within the normal five-minute-past/60-second-future window. A saved signed download token is intentionally a portable reusable capability: its timestamp must be nonzero and remains authenticated, but freshness expiry is not reapplied when the user later downloads it; signature, configured topic, provider, ID, and all attachment metadata are still revalidated before work. `--offer-file` and `--offer-stdin` avoid exposing this reusable plaintext capability in argv and shell history. A raw Iroh `BlobTicket` is also accepted for interoperability, but is treated only as a file and has no meshmsg-signed name, kind, or declared size.
...
offer ID. Download is one ordinary typed IPC operation: the CLI uses
`DownloadMode::Install`, while `meshmsg-web` uses `DownloadMode::Raw` to export the
verified file or deterministic directory tar into its owner-only temporary area. The
daemon's shared bounded operation cache joins exact in-flight duplicates and replays
exact terminal results. A schema-v2 completion carries a digest of the exact token and
is accepted through one shared CLI/web validator only when operation ID, token, offer
...
age/direction/dry-run/limit input, and each download's signed/raw token, typed mode,
plus exact output representation. Changed inputs conflict before
storage/network work. Prune's schema-1 IPC request has no `cutoff_ms`: after the first
...
resolution as cache metadata. Success and partial-error lifecycle-v3 records expose
...
```json
{"type":"attachment_shared","schema_version":3,"operation_id":"0123456789abcdef0123456789abcdef","from":"<peer-id>","message_id":"0123456789abcdef0123456789abcdef","timestamp_ms":1700000000000,"offer_id":"0123456789abcdef0123456789abcdef","source_digest":"<64-lowercase-hex>","kind":"file","name":"report.pdf","size":1234,"ticket":"<blob-ticket>","offer":"<signed-offer>","delivery_acknowledged":false}
{"type":"attachment_offer","schema_version":2,"from":"<peer-id>","message_id":"fedcba9876543210fedcba9876543210","timestamp_ms":1700000000000,"offer_id":"fedcba9876543210fedcba9876543210","kind":"directory_tar_v1","name":"results.tar","size":4096,"ticket":"<blob-ticket>","offer":"<signed-offer>"}
{"type":"download_complete","schema_version":2,"operation_id":"0123456789abcdef0123456789abcdef","token_digest":"<64-lowercase-hex>","offer_id":"<id>","kind":"file","name":"report.pdf","size":1234,"from":"<peer-id>","output":"./received-report.pdf","installed":true,"pinned":true,"destination_synced":true,"cleanup_complete":true,"warnings":[]}
```
