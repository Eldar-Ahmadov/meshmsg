...
All remove, prune, CLI download, and browser export requests carry a reusable
client-generated lowercase 128-bit operation ID distinct from their per-attempt
request IDs and from the selected offer ID. The daemon's shared bounded operation
cache joins exact in-flight duplicates and replays exact terminal results. A
schema-v2 completion carries a digest of the exact token and is accepted through
one shared CLI/web validator only when operation ID, token, offer ID, provider,
kind, name, signed size (when present), and exact output all match the request. It binds
remove selectors, every prune age/direction/dry-run/limit input, and each download's
signed/raw token plus exact output representation. Changed inputs conflict before
