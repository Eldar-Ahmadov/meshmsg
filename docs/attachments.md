...
Copy the `offer` value from the `share` or `listen` JSON output. Signed offers require one canonical lowercase operation-format ID shared by the EnvelopeV3 message ID and embedded offer ID, a canonical raw ticket whose provider and content hash/format are signed, a safe typed name/kind, the signed size and timestamp, and the expected topic. The full signed relationships are revalidated at wire, IPC, saved download-token, and generated-event boundaries; malformed offers are rejected before accepted-traffic or replay/rate admission, subscriber fanout, download registration, network/export work, or pin/tag creation (they still pay the separate cheap pre-verification attempt budget). Persistent tags do not retain the signed envelope and therefore cannot revalidate its signature, topic, timestamp, ticket, or body. Tag parsing instead validates only the canonical provider, offer ID, direction/kind, and encoded name fields that the tag actually stores. Live wire and IPC events require a nonzero signed timestamp within the normal five-minute-past/60-second-future window. A saved signed download token is intentionally a portable reusable capability: its timestamp must be nonzero and remains authenticated, but freshness expiry is not reapplied when the user later downloads it; signature, configured topic, provider, ID, and all attachment metadata are still revalidated before work. `--offer-file` and `--offer-stdin` avoid exposing this reusable plaintext capability in argv and shell history. A raw Iroh `BlobTicket` is also accepted for interoperability, but is treated only as a file and has no meshmsg-signed name, kind, or declared size.
...
meshmsg offers prune [--operation-id <32-lowercase-hex>] [--older-than-secs <SECONDS>] [--direction incoming|outgoing] [--max-delete 512] [--dry-run]
```
...
Listing returns at most the first 512 valid tags in the blob store's ordered tag stream and performs at most 4,096 tag inspections. It uses one valid item of lookahead rather than calculating an exact total. JSON sets `"truncated":true` when more valid entries were observed, the inspection bound was reached, or a per-item store error made completeness uncertain. `"item_errors"` reports the number of per-item store errors encountered; malformed meshmsg tags are ignored. Human output prints an explicit warning whenever the result is truncated or items could not be read. Only one listing runs at once; another fails immediately with `offers_busy`, fixed
public text, `retryable:true`, and `outcome:not_started`. The store API constructs its ordered range result internally before meshmsg consumes this bounded prefix, but listing runs outside the daemon event loop.

## Acceptance and overwrite behavior

Downloads are always explicit. The output path is required and must not exist. Its immediate parent must already exist and be a directory; meshmsg does not create output ancestors. This keeps durability precise: only that existing parent's namespace changes, and meshmsg syncs it after installation. The CLI makes a relative output absolute without filesystem canonicalization, retains that exact OS-string/byte representation, and rejects a correlated completion unless its `output` matches exactly; Path-equivalent dot components and repeated or trailing separators do not match. Empty transfers may emit exactly `received_bytes:0,total_bytes:0`; nonempty progress always has a positive total and never exceeds it.
...
All remove, prune, and download requests carry a reusable client-generated lowercase
128-bit operation ID distinct from their per-attempt request IDs and from the selected
offer ID. Download is one ordinary typed IPC operation. The sole supported mode,
`DownloadMode::Install`, installs a verified file or extracts a deterministic directory archive. The daemon's shared bounded operation cache joins exact in-flight duplicates and replays
exact terminal results. A canonical completion carries a digest of the exact token and
is accepted through the shared validator only when operation ID, token, offer
ID, provider, kind, name, signed size (when present), and exact output all match the
request. The operation fingerprint binds remove selectors, every prune
age/direction/dry-run/limit input, and each download's signed/raw token plus exact
output representation. Changed inputs conflict before storage/network work. The canonical prune IPC request has no `cutoff_ms`: after the first
stable-intent cache admission, the daemon resolves `cutoff_ms =
daemon_now_ms.saturating_sub(older_than_secs.saturating_mul(1000))` and retains that
resolution as cache metadata. Success and partial-error lifecycle records expose
the authoritative effective age and cutoff. Consequently an identical real CLI retry
needs only the same visible command and operation ID, uses the original
selection/result rather than deleting the next batch, and retrying remove reports
the authoritative original counts even though the end state is already safe.
...
Attachment offers use the topic-bound broadcast envelope V3. Only that current signed shape is decoded; V2 and malformed signed tokens fail closed generically. The decoded outer envelope is capped at 65,536 bytes and its unpadded base64url token at 87,382 bytes. Raw Iroh `BlobTicket` values remain accepted as described above. V3 daemons use `/meshmsg/broadcast-gossip/3`, so pre-V3 peers neither receive nor inject V3 attachment offers.
...
```json
{"protocol_version":3,"request_id":"0123456789abcdef0123456789abcdef","type":"attachment_shared","operation_id":"0123456789abcdef0123456789abcdef","from":"<peer-id>","message_id":"0123456789abcdef0123456789abcdef","timestamp_ms":1700000000000,"offer_id":"0123456789abcdef0123456789abcdef","source_digest":"<64-lowercase-hex>","kind":"file","name":"report.pdf","size":1234,"ticket":"<blob-ticket>","offer":"<signed-offer>","delivery_acknowledged":false}
{"protocol_version":3,"request_id":"0123456789abcdef0123456789abcdef","type":"attachment_offer","from":"<peer-id>","message_id":"fedcba9876543210fedcba9876543210","timestamp_ms":1700000000000,"offer_id":"fedcba9876543210fedcba9876543210","kind":"directory_tar_v1","name":"results.tar","size":4096,"ticket":"<blob-ticket>","offer":"<signed-offer>"}
{"protocol_version":3,"request_id":"0123456789abcdef0123456789abcdef","type":"download_complete","operation_id":"0123456789abcdef0123456789abcdef","token_digest":"<64-lowercase-hex>","offer_id":"<id>","kind":"file","name":"report.pdf","size":1234,"from":"<peer-id>","output":"./received-report.pdf","mode":"install","installed":true,"pinned":true,"destination_synced":true,"cleanup_complete":true,"warnings":[]}
```
