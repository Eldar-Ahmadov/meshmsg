...
| `direct-replay-v2-*.wal` | 8 MiB; 512 bytes per record | WAL version 2; each read/repair or capacity-check/append operation uses its same no-follow/reparse-aware checked handle. Append capacity is reserved for the confirmation transition before the first record, and post-sync identity checking detects path replacement. Only an unterminated final fragment of at most 512 bytes is truncated; a 513-byte tail and complete malformed/checksum-invalid records remain untouched and fail closed. |
| `config.json.v0.bak` | 64 KiB | Exact pre-migration bytes, created once with owner-only permissions. A differing existing backup aborts migration. |
