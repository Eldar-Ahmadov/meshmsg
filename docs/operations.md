...
An invite is effectively a topic-access capability. Anyone who obtains it can join the topic, read plaintext broadcast messages and attachment offers, and send signed broadcasts. Broadcasts and attachment offers remain topic-wide plaintext, but their wire format changed incompatibly to the topic-bound V3 envelope and `/meshmsg/broadcast-gossip/3`; do not mistake the private-send feature for encryption of the topic-wide swarm.
...
The dedicated blocking worker owns the map and all filesystem I/O behind a bounded 64-request queue; async protocol tasks hold no synchronous persistence lock. Every 256 WAL records and at clean shutdown, it atomically commits a checksummed compact snapshot before atomically resetting the WAL. Recovery tolerates an old WAL over a newer snapshot, truncates and syncs only an incomplete final append, and fails closed on malformed complete records, checksums, bindings, transitions, conflicts, or oversized state. A terminal worker error sets status to `direct_replay_available:false` with stable `direct_replay_error:"direct replay persistence worker failed"`; a triggering pre-append failure returns signed `Unavailable` while an append/sync ambiguity returns signed `DeliveryOutcomeUnknown`; queued/subsequent requests receive signed `Unavailable`, and shutdown reports the worker failure.
...
The daemon does not mirror protocol events to process stdout. `--json` does not change daemon output into an NDJSON event stream: clients consume the authenticated, bounded subscription through `meshmsg listen` (or another `subscribe` IPC client). The daemon writes one startup line to stderr after networking and local IPC are ready, and top-level fatal startup/runtime errors also use stderr.

There is no process-wide diagnostic queue, terminal-writer thread, output telemetry, or custom panic hook. Routine malformed traffic and background-task failures do not become terminal output. One-shot commands and streaming clients use ordinary stdout for their documented results; one-shot human errors use stderr and one-shot JSON errors remain a single canonical stdout object. Daemon shutdown remains nonblocking because it has no event-output queue to drain; IPC admission is still closed and bounded client tasks are drained or aborted before network teardown.
...
Broadcast application envelopes are limited to exactly 65,536 serialized bytes. Text bodies must be nonempty and are limited to 65,358 UTF-8 bytes for local production, remote acceptance, typed IPC, CLI input, and subscription output. The 178-byte reserve is the measured and boundary-tested worst-case EnvelopeV3 postcard metadata/signature overhead, including maximum-width timestamp and body-length varints; a 65,359-byte worst-case body produces exactly 65,537 bytes and is rejected. Each complete envelope is one Iroh Gossip application message. Iroh `max_message_size` is 66,048 bytes: the exact 65,536-byte application bound plus 512 bytes, which conservatively exceeds Iroh 0.101's postcard message header while remaining an explicit finite transport bound. Iroh may cache and forward an item before meshmsg validates its signature, topic, body, timestamp, or replay identity. This pre-validation cache/fanout tradeoff is explicitly accepted for the invite-controlled swarm in favor of a single-message transport. Once delivered to meshmsg, authenticated-source byte/count budgets run before decoding, and no replay admission or local subscriber fanout occurs until complete signature, topic, body, attachment, and freshness validation succeeds. The 64-entry subscriber ring therefore retains less than 4 MiB of broadcast body bytes (plus fixed typed-event metadata) before reporting lag. Invalid local bodies are rejected before operation-cache, wire, and message-event side effects; invalid signed remote text or attachment semantics are rejected before accepted-traffic/replay admission and fanout, while still paying the separate pre-verification attempt bound. Private bodies remain independently limited to 4,096 UTF-8 bytes, with a bounded 6 KiB direct protocol frame. Incoming private acceptance uses bounded connection, volatile delivery, and replay-persistence queues and can return a signed `Busy` rejection under load. The resulting local `private_recipient_busy` failure is terminal and cached for that operation ID; after load subsides, a fresh attempt uses a new operation ID. A local `command_timeout` means the daemon released that IPC connection, not necessarily that already-submitted work was cancelled; for mutating commands the outcome may be unknown. Stop is stricter: the daemon reserves bounded command-queue capacity and submits the stop command before returning the unit `stopping` acknowledgement; queue timeout or closure returns an error with `outcome:"not_started"` and never a false success.
...
| State | Maximum read | Version/recovery |
|---|---:|---|
| `config.json` | 64 KiB | Top-level `schema_version: 1` is required; missing, unknown, and future versions fail closed. |
...
| `alias.json` | 4 KiB | Version 1 with a matching identity binding is required; absence and unknown versions fail closed. |
| `attachment-retention-v1.json` | 8 MiB and 8,192 entries | Schema version 1. Absence initializes an empty index only when no managed attachment pins exist; managed pins without the index fail closed. A valid current index is reconciled against authoritative pins. |
...
Lock and socket files are never loaded as structured state. Attachment payloads and the Iroh blob database are streamed or accessed through Iroh rather than whole-file state reads.

**Breaking local-state change:** pre-current local formats are no longer imported. An unversioned v0 `config.json` and a missing or unbound `alias.json` now stop startup/diagnosis instead of being upgraded or interpreted as an alias opt-out. A directory containing only `direct-replay-v1-*.json` starts with an empty current replay store; the obsolete file is ignored and is not renamed, backed up, or deleted. Existing `config.json.v0.bak` files are likewise unused and left untouched. A missing attachment retention index is accepted only for a fresh store with no managed pins; if managed pins already exist, startup fails with instructions to restore the current index or remove the pins using a compatible older daemon instead of inventing creation timestamps. Upgrade through an intermediate release first if old local state must be retained. EnvelopeV3 and `/meshmsg/broadcast-gossip/3` remain hard broadcast compatibility breaks. Local IPC protocol v4 is a separate hard break that removes ambiguous/dead frame variants, splits lifecycle result DTOs, makes stop acknowledgement unit-typed, and removes inactive HTTP/web error codes. Older local clients and daemons are rejected without fallback, so reinstall and restart every client and daemon together. V2 broadcasts and signed attachment tokens likewise remain rejected. Current broadcasts are uniformly limited to 65,358 bytes.

Current-format corruption still fails closed. Version probes decode unsigned 64-bit values before schema-specific narrow fields, so future values such as 256 are unsupported rather than misclassified as parse failures. Replay snapshot and attachment-index vectors use bounded visitors that reject an extra entry and overlong sender, ID, fingerprint, recipient, topic, checksum, or tag key before growing the retained collection. Before serde decoding, an allocation-free lexical pass incrementally counts every JSON string's decoded UTF-8 length, including backslash escapes, Unicode escapes, and surrogate pairs, so escaped amplification cannot create unbounded parser scratch before those field-specific checks. Persistent-state diagnostics name the state class and error category (missing, too large, I/O, parse, corruption, or unsupported version); public JSON errors remain canonical and do not expose local paths, key material, or message bodies. These checks prevent pathname races from producing false persistence or replay success, but they are not isolation from another process with the same account's state-directory access: such a process can still cause fail-closed unavailability or unknown delivery outcomes, and direct in-place tampering is detected on recovery by strict parsing/checksums rather than prevented as an authorization boundary.

## Windows

Windows supports the same commands. Keep `meshmsg daemon` running in a dedicated PowerShell window.
...
[Service]
Type=simple
ExecStart=%h/.cargo/bin/meshmsg daemon
Restart=on-failure
RestartSec=5
