...
RAII closes daemon stdout admission and waits at most 500 ms on every return or unwind path before network resources are dropped; normal shutdown closes and drains explicitly before fallible router/replay teardown. No later event admission succeeds. A timed-out isolated writer is abandoned so shutdown cannot hang. Diagnostic startup and daemon-output startup failures abort production startup rather than silently degrading. `meshmsg diagnostics` uses separately negotiated `diagnostic_status_v3` for independently named stdout and diagnostic occupancy/capacity/high-water marks, strict output-admission rejections, all drop classes, accepted/sampled/suppressed/written counts, writer loss/failure/panic/health, and process panic count. Current clients prefer v3 and intentionally fall back to the exact v0.1.19 `diagnostic_status_v2` contract, which omits admission rejections; current daemons advertise and serve both versions. Independent maxima are never added because their peaks may occur at different times.
