...
Current clients use a separate derived presence topic, direct-message protocol `/meshmsg/direct/2`, and the topic-bound `/meshmsg/broadcast-gossip/2` protocol. Pre-V2 peers do not exchange broadcasts or attachment offers with V2 peers; this deliberate protocol separation prevents legacy envelopes from entering the V2 receive path. Legacy signed attachment tokens are explicitly rejected because they are not topic-bound, while raw Iroh `BlobTicket` downloads remain supported. Older peers may still publish presence, but they do not accept `/meshmsg/direct/2` or its signed result semantics. A private send to an old peer therefore fails rather than falling back to plaintext broadcast.
