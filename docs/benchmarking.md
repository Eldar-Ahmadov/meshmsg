...
After a started record, disconnects, malformed records, and daemon terminal errors still produce one correlated partial summary from the latest validated snapshot. Daemon-produced terminal summaries set `accounting_complete:true`; a client-synthesized snapshot sets it false and does not invent attempts or missed slots after its last validated record. Any following JSON error retains the benchmark request ID. Disconnects and invalid responses use `outcome:partial`; a valid strict daemon error envelope is preserved unchanged. A valid `send_failed` summary is followed by the stable `send_failed`/`partial` error in CLI JSON mode. These errors describe a benchmark that started and never regress to `outcome:not_started`.
