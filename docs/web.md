...
- **Broadcast:** first negotiates `idempotent_mutations_v1` and fails closed before submission if the daemon does not advertise it, then submits once per user action. The tab generates an operation ID and retains it with an unchanged draft after an unknown response; manually submitting that same draft again reuses the ID so the current daemon can return its cached outcome without another broadcast. A successful response means **queued locally; delivery is unconfirmed and not acknowledged**. The UI, HTTP bridge, IPC, and daemon producer share the nonempty 3900-UTF-8-byte body limit. Wire/SSE event consumers retain compatibility with valid released V2 bodies through the exact 3928-byte worst-case envelope capacity. Invalid HTTP text returns canonical `invalid_message`/`not_started` with the submitted operation ID before throttling or daemon contact.
...
- **Live feed:** newest 100 entries in this tab; incoming text, daemon-confirmed local queued sends, attachment information cards, sanitized peer-directory changes, and gap notices. Incoming broadcast messages and offers use `schema_version:2`; local queued sends and outgoing shares use schema 3 and carry matching `operation_id`/V2 `message_id`; the bridge rejects legacy or malformed versions instead of rewriting them as V2. Each subscription is uninitialized until it receives a reconstructable `connected` event with `endpoint_online:true` and then a valid atomic peer snapshot; only then are discovered/updated/expired events applied. A malformed/non-public handshake produces one correlated sanitized `invalid_daemon_response` SSE error and closes cleanly. IPC connection establishment and the connected frame share one eight-second bridge deadline; connection time is deducted from the first-frame budget. An out-of-order lifecycle event, invalid/missing startup snapshot, disconnect, lag, or hidden tab immediately invalidates the current-peer summary and reconnects rather than comparing against an old directory. Identical presence refreshes do not create feed entries. Callbacks and timers from superseded subscriptions, and status replies started before a disconnect or visibility-generation change, are ignored. A sampled daemon-generated event contract failure appears as a safe, text-only `internal_contract_error` warning card with suppression accounting; malformed attachment-family IPC records are independently limited to one such diagnostic per second with exact suppression accounting. Neither closes nor reconnects the feed, and invalid/cross-topic/stale offers never create download handles. Incoming attachment cards include an explicit browser Download control when the connected daemon advertises `web_download_v1`; an older running daemon leaves cards informational until it is upgraded/restarted. Files retain their offered name and directory snapshots download as their deterministic `.tar` archive. After preparation, the UI presents a real **Save file** link that requires another user click. Signed offers remain only in bounded, short-lived server memory and are represented in SSE/API by opaque retryable IDs—raw offer tokens, blob tickets, and server paths never enter the DOM. Outgoing cards remain informational. Every connected web tab subscribes independently, so sends from CLI, chat, or another web tab and shares from local CLI/processes appear in all currently connected live feeds with the daemon's canonical sender and timestamp. The sending tab does not add an optimistic copy: the canonical entry is the same daemon event every tab receives. If that event falls in a disconnect/lag gap, it is not reconstructed from the HTTP reply. The feed has no history, replay, or delivery receipts. Untrusted names, senders, and messages containing markup are displayed literally, without links or HTML interpretation.
...
- 32 HTTP headers, 16 KiB header buffer; 25,600-byte JSON body/frame bounds and a nonempty, 3900-byte UTF-8 send body bound.
- 5-second ordinary JSON body-read timeout; one combined 8-second IPC connection-plus-connected-frame timeout for subscription startup; UI message submit timeout 12 seconds. Attachment upload/share operations are hard-limited to 70 minutes, and HTTP connections to 75 minutes. Client disconnect/timeout does not cancel an already-submitted daemon action.
...
A successful browser preparation uses the daemon's normal verified download path and creates the same persistent `meshmsg/in/v1/...` blob pin as a CLI download. The HTTP temporary file expires independently. The blob pin survives daemon restarts until the configured daemon retention pass or an operator runs `offers remove`/`offers prune`; uploads/download preparation can fail with authoritative quota or minimum-free-space errors. `/api/status` exposes only cached aggregate attachment usage, quota, sampled free-space/time, retention, and pressure fields—never tag names or paths. Attachment upload/download failures crossing HTTP are validated against the daemon's strict versioned lifecycle-error DTO; malformed versions, fields, codes, outcomes, or operation IDs are replaced by a bounded internal/unknown error. Public responses preserve stable code, outcome, retryability, operation/offer IDs, and partial counts, but replace daemon diagnostics with fixed code-specific messages so state paths, staging paths, database details, and internal causes remain only in local web-process diagnostics. Lifecycle mutation remains CLI/owner-only IPC and has no HTTP route.
...
The fake-daemon HTTP harness uses Unix sockets (Linux/macOS) and a debug-only offline fixture signer; it starts no production networking or blob-storage daemon. It checks allowlists, Host/Origin, asset security headers, simultaneous-feed synchronization, malformed-offer diagnostic/feed continuity/no-handle behavior, bounded upload retry/conflict and ambiguous-result caching, malformed download completions, definitely-not-started handle reuse, interrupted/ranged downloads, late-export restart cleanup, capability/path filtering, SSE framing/capacity/cleanup, request bounds/timeouts, queued/rejected/unknown outcomes, no send retry, and independent shutdown. It also replaces a stopped daemon's state and topic under the same directory while the web process remains alive, then proves a reconnected feed rejects the old-topic offer and delivers correlated new-topic offer/share events. The real-peer harness creates temporary isolated states, starts two actual daemons, checks a browser upload and canonical guarded share events plus verified browser downloads of a zero-byte file, regular file, and deterministic directory tar, and cleans up. It requires working Iroh networking and does not change Tailscale configuration.
