...
- 16 in-flight ordinary IPC request/reply operations, 16 live feeds, two concurrent attachment downloads, and bounded short-lived offer/job registries; excess operations return 503.
...
- 32 buffered events per live feed; 10-second SSE comments; 5-second event-channel backpressure timeout; maximum HTTP connection lifetime 1 hour; ready downloads advertise byte ranges, so a larger/slower interrupted transfer can resume during the ready-file TTL. A slow SSE reader is still disconnected by event-channel backpressure rather than retaining unbounded history.
