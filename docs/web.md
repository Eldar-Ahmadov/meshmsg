...
- **Identity/status:** daemon identity, endpoint availability, topic join state, and neighbor count. None proves delivery. The web UI omits local paths, invites, and attachment metadata.
...
- **Live feed:** newest 100 entries in this tab, text only; incoming text, daemon-confirmed local queued sends, peer changes, and gap notices. Every connected web tab subscribes independently, so sends from CLI, chat, or another web tab appear in all live feeds with the daemon's sender and timestamp. The sending tab does not add an optimistic copy: the canonical entry is the same daemon event every tab receives. If that event falls in a disconnect/lag gap, it is not reconstructed from the HTTP reply. The feed has no history, replay, delivery receipts, or attachment operations. Messages containing markup are displayed literally, without links or HTML interpretation.
...
| `GET /api/events` | One local IPC `subscribe` connection streamed as SSE, including incoming messages and local queued sends |

No filesystem, share/download/offer, benchmark, stop, invite, or topic commands are exposed. A write requires `Content-Type: application/json` (without encoding) and an exact same-origin `Origin` matching an allowed `Host`. GETs require an allowed Host and, when present, a matching Origin. Cross-site browser fetches are rejected. There is no permissive CORS/preflight support. Duplicate Host/Origin values fail closed. `Forwarded`, `X-Forwarded-*`, and Tailscale identity headers are **not** trusted or used for authorization. The reverse proxy must preserve the configured Host. A 403 behind Serve indicates an origin/Host mismatch: fix the explicit configuration, not the protections.

Responses use `Cache-Control: no-store`, `nosniff`, no-referrer, frame denial and a CSP forbidding inline scripts, third-party resources, forms, and framing. Assets are same-origin. Untrusted messages only reach `textContent`.
...
The fake-daemon HTTP harness uses Unix sockets (Linux/macOS). It checks allowlists, Host/Origin, asset security headers, simultaneous-feed synchronization for web and local CLI sends, SSE framing/filtering/capacity/cleanup, request bounds/timeouts, queued/rejected/ambiguous outcomes, no send retry, offline/restart, and independent shutdown. The real-peer harness creates temporary isolated states, starts two actual daemons, checks canonical queued events in two web feeds plus web-to-peer and peer-to-both-feeds receipt, and cleans up. It requires working Iroh networking and does not change Tailscale configuration.
