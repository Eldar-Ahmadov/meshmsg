...
- Follow-up finding 4 is substantially implemented but remains open pending the documented prune retry ergonomics, final prune request-contract reconciliation, and documentation review. One canonical error-code specification now binds fixed public text, outcome/retryability pairs, field policy, and operation applicability; bounded nonblocking local diagnostics retain private causes before sanitization. Shared producer/consumer DTOs cover offers and lifecycle successes. Lifecycle v3 repeats and validates exact selectors, effective age, client-resolved explicit cutoff, dry-run, maximum, and selection/count/byte semantics through negotiated `attachment_lifecycle_v3`; offers v1 enforces the producer/documented 512-entry cap and applicable truncation invariants. Focused tests exhaust an independently enumerated mutation-consumer error matrix, malformed combinations, producer/cache/normalizer request mismatches, age-bound cutoff arithmetic/replay clocks, status-v1 compatibility, and 512/513 plus real-store 4095/4096/4097 listing boundaries.
